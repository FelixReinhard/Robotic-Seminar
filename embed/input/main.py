...
pico_led.off()
while not 0x5a in i2c.scan():
    print("Cant find mpr121... Waiting")
    sleep(1)
mpr = MPR121(i2c)
...
# purple 1, white = 2, yellow = 3, two black = 4, 
while not wlan.isconnected():
    print("waiting to connect to wlan")
    sleep(.25)

addr_info = socket.getaddrinfo("192.168.43.253", 1269)
addr = addr_info[0][-1]
s = socket.socket()
s.connect(addr)
...
try:
    while True:
        to_send = [0] * 12
        for i in range(12):
            to_send[i] = 1 if mpr.is_touched(i) else 0
            
            if mpr.is_touched(i):
                print(f"Pin {i}: ", mpr.is_touched(i))
        s.send(bytes(to_send))
        print(mpr.is_touched(2))
        #b = s.recv(1)
        #if b == 2: # stop signal
            #break
except:
